output$plot_mediana_usd_por_m2_za <- renderPlot({
  ...
  datos_filtrados <- archivo %>%
    filter(
      ...
      zona_agrupada %in% input$depto_mediana_za, 
      fecha %in% input$depto_mediana_fecha
    ) %>%
  ...
  ggplot(datos_filtrados, aes(x = fecha, y = dolares_m2total, color = zona_agrupada, group = zona_agrupada)) +
    ...
    labs(title = "Mediana de Precio por m² (USD) de departamento en Venta por Localidad",
         x = "Fecha",
